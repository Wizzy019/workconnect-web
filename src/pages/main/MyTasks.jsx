  ...
  const fetchData = async () => {
    ...
    if (user) {
      const { data: tasksData, error } = await supabase
        .from("workconnect_tasks")
        .select("*")
        .eq("worker_id", user.id);

      const { data: gigsData, gigError } = await supabase
  ...
  return (
    <div className="min-h-screen bg-[#a3ffcd28] pb-10">
      ...
      <div className="max-w-xl mx-auto px-6 py-6 flex items-center border-b border-gray-100 bg-white sticky top-0 z-10">
        <h1 className="flex-1 text-center text-2xl font-bold text-[#001e2b] mr-8">
          My Tasks
        </h1>
      </div>
      ...
      <div className="max-w-xl mx-auto px-6 pt-8 space-y-10">
        <section className="space-y-4">
          <h2 className="text-lg font-bold text-[#001e2b]">
        ...
        <section className="space-y-4">
          <div className="flex justify-between items-center">
            ...
            <button
              className="text-[#1dbf73] border border-[#1dbf73] px-4 py-1.5 rounded-lg text-sm font-semibold hover:bg-[#1dbf73] hover:text-white transition-all"
          ...
          {gigs.length > 0 ? (
            <div className="space-y-6">
              {gigs.map((gig) => (
                <div
                  key={gig.id}
                  className="relative bg-white border border-gray-200 rounded-xl overflow-hidden p-4 shadow-sm"
                >
                  ...
                  <TalentCard
                    ...
                    onUpdate={(data) => handleUpdate(gig.id, data)}
                  />
