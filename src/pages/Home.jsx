...
function Home() {
  return (
    <>
      <Navbar />
      <section
        className="w-full h-screen md:h-96 bg-center bg-cover relative"
        style={{
          backgroundImage: `linear-gradient(rgba(0,9,0,0.5),rgba(0,9,0,0.9)),url(${BackgroundImage})`,
        }}
      >
        <div
          className="w-full flex items-center justify-center flex-col absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 
        gap-4 p-2 text-center"
        >
          <h1 className="text-4xl font-bold text-white ">
            Connect With Real Work. Build Real Income
          </h1>
          <h3 className="text-2xl font-semibold text-white">
            WorkConnect links skilled people to real-time job opportunities —
      ...
      <section className="md:min-h-84 text-center text-pretty px-2 py-6 bg-[#e0ffe9]">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 mt-2">
          <BulletPoint
            icon={faCheckCircle}
            label="Verified opportunities only — no fake jobs, no long stories"
          />
          <BulletPoint
            icon={faUsers}
            label="Professional connections — link up with clients who actually pay"
          />
          <BulletPoint
            icon={faDiagramProject}
            label="Simple workflow — sign up, apply, get hired, done"
          />
          <BulletPoint
            icon={faGlobeAfrica}
            label="Built for Africa's workforce — reliable, transparent, and easy to use"
          />
        </div>
      </section>
      <section
      ...
      <section
        className="md:h-96 flex flex-col items-center justify-center bg-center bg-cover relative p-4"
        style={{
      ...
      <section
        id="footer"
        className="min-h-screen flex flex-col justify-between bg-[#e0ffe9]"
      >
        <div className="flex md:flex-row items-start p-2">
          <div className="w-full p-4 flex flex-col md:flex-row md:items-center justify-evenly items-start">
            ...
            <div className="text-xl font-semibold p-4">
              Workconnect connects skilled professionals with real job
              opportunities,
              <br /> making hiring and getting hired simple, fast, and reliable.
            </div>
          </div>
        </div>
